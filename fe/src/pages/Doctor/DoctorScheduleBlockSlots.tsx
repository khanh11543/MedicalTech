...
export default function DoctorScheduleBlockSlots() {
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      try {
        setLoading(true);
        setError(null);
        const slots = await doctorScheduleService.listTimeSlots(
          dateRange.startDate,
          dateRange.endDate
        );
        setTimeSlots(slots);
        ...
        slots.forEach((slot: TimeSlotDTO) => {
  ...
  const handleBlockSlot = async (reason: string) => {
    ...
    try {
      ...
      await doctorScheduleService.blockTimeSlot(blockingSlotId, reason);
      ...
      const errorMsg =
        err instanceof Error ? err.message : 'Failed to block slot';
      toast.error(errorMsg);
    }
  };
  ...
  const handleUnblockSlot = async (slotId: number) => {
    try {
      await doctorScheduleService.unblockTimeSlot(slotId);
      ...
      const errorMsg =
        err instanceof Error ? err.message : 'Failed to unblock slot';
      toast.error(errorMsg);
    }
  };
  ...
  return (
    <>
      <PageMeta
        ...
        description='Block or unblock your appointment slots'
      />
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-gray-600 dark:text-gray-400 mt-2'>
                Manage your availability by blocking slots for emergencies,
                meetings, or personal time
              </p>
            </div>
        ...
        {loading ? (
          ...
          <div className='rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-12'>
            <div className='flex flex-col items-center justify-center text-center'>
              ...
              <p className='text-gray-600 dark:text-gray-400'>
                Generate time slots from your weekly schedules to manage them
                here
              </p>
            </div>
          </div>
              ...
              >
                ...
                <div className='px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50'>
                  ...
                  >
                    ...
                    <div className='flex items-center gap-4'>
                      <div className='text-sm text-gray-600 dark:text-gray-400'>
                        {day.timeRanges.reduce(
                          (sum, range) => sum + range.slots.length,
                          0
                        )}{' '}
                        slots
                      </div>
                      <div
                        className={`text-xl transition-transform ${expandedDays.has(day.date) ? 'rotate-180' : ''}`}
                      >
                        <svg
      ...
      {selectedSlot && (
        <BlockSlotModal
          ...
          slotDate={new Date(
            selectedSlot.slotDate + 'T00:00:00'
