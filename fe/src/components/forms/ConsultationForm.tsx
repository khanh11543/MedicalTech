...
export default function ConsultationForm({ appointmentId, draftId, onBack, onDraftSaved }: ConsultationFormProps) {
    const { showToast } = useToast();
    const [loading, setLoading] = useState(false);
    const [saving, setSaving] = useState(false);
    const [tabActive, setTabActive] = useState<"examination" | "amendments">("examination");
    ...
    const [amendments, setAmendments] = useState<Array<{
        id: string;
        content: string;
        createdAt: string;
        signedAt?: string;
        ...
        const loadConsultation = async () => {
            ...
            try {
                ...
                if (draftId) {
                    ...
                    consultation = await consultationService.getConsultation(appointmentId);
                    setFormData(consultation);
                    setIsFinalized(consultation.status === "FINALIZED");
                    
                    if (consultation.status === "FINALIZED") {
                        const amendments = await consultationService.getAmendments(appointmentId);
                        setAmendments(amendments as any);
                    }
                }
            } catch (error) {
    ...
    }, [formData.status, isFinalized, formData]);
    ...
    const handleFinalize = async () => {
        ...
        setSaving(true);
        try {
            const finalizedConsultation = await consultationService.finalize(formData);
            setFormData(finalizedConsultation);
            setIsFinalized(true);
            showToast("Record finalized and appointment approved", "success");
            ...
            showToast("Error finalizing record", "error");
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleAddAmendment = async () => {
        ...
        setSaving(true);
        try {
            const amendment = await consultationService.addAmendment(appointmentId, newAmendment);
            setAmendments(prev => [...prev, amendment as any]);
            setNewAmendment("");
    ...
    const handleSignAmendment = async (amendmentId: string) => {
        setSaving(true);
        try {
            const signedAmendment = await consultationService.signAmendment(appointmentId, amendmentId);
            setAmendments(prev =>
                prev.map(a =>
                    a.id === amendmentId
            ...
            showToast("Error signing amendment", "error");
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleVitalChange = (vital: string, value: number | null) => {
        ...
        if (vital === "height" || vital === "weight") {
            if (updatedVitals.height && updatedVitals.weight) {
                ...
                updatedVitals.bmi = Math.round((updatedVitals.weight / (heightInMeters * heightInMeters)) * 10) / 10;
            }
        }
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            for (const file of Array.from(files)) {
                const attachment = await consultationService.uploadAttachment(appointmentId, file);
                setFormData(prev => ({
                    ...prev,
                ...
                <div className="space-y-8">
                    ...
                    <Section title="3. Vital Signs" required>
                        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                            <div>
                                ...
                                <InputField
                                    ...
                                    onChange={(e) => handleVitalChange("temperature", e.target.value ? parseFloat(e.target.value) : null)}
                                    disabled={isFinalized}
                                />
                            </div>
                            <div>
                                ...
                                <InputField
                                    ...
                                    onChange={(e) => handleVitalChange("systolic", e.target.value ? parseInt(e.target.value) : null)}
                                    disabled={isFinalized}
                                />
                            </div>
                            <div>
                                ...
                                <InputField
                                    ...
                                    onChange={(e) => handleVitalChange("diastolic", e.target.value ? parseInt(e.target.value) : null)}
                                    disabled={isFinalized}
                                />
                            </div>
                            <div>
                                ...
                                <InputField
                                    ...
                                    onChange={(e) => handleVitalChange("heartRate", e.target.value ? parseInt(e.target.value) : null)}
                                    disabled={isFinalized}
                                />
                            </div>
                            <div>
                                ...
                                <InputField
                                    ...
                                    onChange={(e) => handleVitalChange("respiratoryRate", e.target.value ? parseInt(e.target.value) : null)}
                                    disabled={isFinalized}
                                />
                            </div>
                        </div>
                        ...
                        <div className="mt-6 border-t border-gray-200 pt-6 dark:border-gray-700">
                            <h4 className="mb-4 font-medium text-gray-900 dark:text-white">Body Mass Index (BMI)</h4>
                            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                                <div>
                                    ...
                                    <InputField
                                        ...
                                        onChange={(e) => handleVitalChange("height", e.target.value ? parseInt(e.target.value) : null)}
                                        disabled={isFinalized}
                                    />
                                </div>
                                <div>
                                    ...
                                    <InputField
                                        ...
                                        onChange={(e) => handleVitalChange("weight", e.target.value ? parseFloat(e.target.value) : null)}
                                        disabled={isFinalized}
                                    />
                                </div>
                                {formData.vitals.bmi && (
                                    <div className="rounded-lg bg-blue-50 p-4 dark:bg-blue-900/20">
                                        <div className="text-xs font-medium text-gray-600 dark:text-gray-400">BMI</div>
                    ...
                    <Section title="5. Diagnosis" required>
                        <div className="space-y-4">
                            ...
                            />
                            <div>
                                <Label htmlFor="diagnosticCode">ICD-10 Code (if applicable)</Label>
                                <InputField
                                    id="diagnosticCode"
                                    placeholder="e.g: J00.9"
                                    value={formData.diagnosticCode}
                                    onChange={(e) => handleFieldChange("diagnosticCode", e.target.value)}
                                    disabled={isFinalized}
                                />
                            </div>
                        </div>
                    ...
                    <Section title="7. Follow-up Instructions">
                        <TextArea
                            ...
                            onChange={(value) => handleFieldChange("followUpInstructions", value)}
                            disabled={isFinalized}
                            rows={3}
                        />
                    </Section>
                    ...
                    <Section title="8. Attachments">
                        <div className="rounded-lg border-2 border-dashed border-gray-300 p-6 text-center dark:border-gray-600">
                            <svg className="mx-auto h-12 w-12 text-gray-400 dark:text-gray-500" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                                <path d="M28 8H12a4 4 0 00-4 4v20a4 4 0 004 4h24a4 4 0 004-4V20m-14-12l6 6m-6-6v12" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                        ...
                        {formData.attachments.length > 0 && (
                            <div className="mt-4 space-y-2">
                                {formData.attachments.map((attachment) => (
                                    <div key={attachment.id} className="flex items-center justify-between rounded-lg bg-gray-50 p-3 dark:bg-gray-700">
                                        <div>
                ...
                <div className="space-y-6">
                    <div className="rounded-lg bg-blue-50 p-4 dark:bg-blue-900/20">
                        ...
                        <p className="mt-1 text-sm text-blue-800 dark:text-blue-400">
                            The finalized record cannot be directly edited. You can add new amendments to update information without modifying the original record.
                        </p>
                    </div>
                    ...
                    {showAmendmentModal && (
                        <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-800 dark:bg-yellow-900/20">
                            <h4 className="mb-3 font-medium text-gray-900 dark:text-white">Add New Amendment</h4>
                            <TextArea
                    ...
                    {amendments.length > 0 && (
                        <div className="space-y-4">
                            <h4 className="font-medium text-gray-900 dark:text-white">Amendment List</h4>
                            {amendments.map((amendment, index) => (
                                <div key={amendment.id} className="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
