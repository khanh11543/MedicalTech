...
export default function UserDropdown() {
  ...
  useEffect(() => {
    if (user) {
      userSettingsService.getProfile().then((profile) => {
  ...
  useEffect(() => {
    const handleAvatarUpdate = (e: Event) => {
      ...
      setAvatarUrl(getAvatarUrl(customEvent.detail));
    };
    ...
    <div className="relative">
      ...
      >
        <div>
          ...
          <span className="mt-0.5 inline-block text-theme-xs text-brand-500 bg-brand-50 dark:bg-brand-500/10 px-2 py-0.5 rounded-full">
            {primaryRole}
          </span>
        </div>
        ...
        <ul className="flex flex-col gap-1 pt-4 pb-3 border-b border-gray-200 dark:border-gray-800">
          <li>
            <DropdownItem
              ...
              tag="a"
              to={user?.roles?.includes("RECEPTIONIST") ? "/receptionist/settings" : "/admin/settings"}
